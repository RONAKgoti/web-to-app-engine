...
class _MainShellState extends ConsumerState<MainShell> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // NATIVE BOTTOM NAVIGATION BAR (Selective Hiding with Intelligent Domain Awareness)
      bottomNavigationBar: (() {
           final current = webState.currentUrl?.toLowerCase() ?? "";
           final target = webState.targetUrl?.toLowerCase() ?? "";
           final initial = webState.initialUrl?.toLowerCase() ?? "";
           
           final ecomDomains = ["flipkart", "nykaa", "myntra", "amazon", "ajio", "tatacliq", "meesho", "snapdeal", "bewakoof", "ebay", "aliexpress", "etsy", "shein"];
           
           bool isEcomMatch(String u) => ecomDomains.any((domain) => u.contains(domain));
           final isEcom = isEcomMatch(current) || isEcomMatch(target) || isEcomMatch(initial);
           
           // FAIL-SAFE: If URL is not yet established, default to HIDING the bar to prevent flickering gaps.
           if (current.isEmpty && target.isEmpty) return false;

           if (!isKeyboardVisible && navItems.length > 1 && !webState.hasNativeWebBar && !isEcom) {
               return true;
           }
           return false;
      })() ? Container(
              ...
              child: SafeArea(
                top: false, 
                child: Container(
                  height: 68, // Stable Premium Height
                  ...
                  child: Row(
                    ...
                    children: navItems.map((item) {
                      ...
                      final bool isActive = (item.label == webState.activeMenuLabel) || (normCurrent == normItem && normItem != "ROOT");
                              ...
                              TweenAnimationBuilder<double>(
                                ...
                                builder: (context, scale, child) {
                                  return Transform.scale(
                                    scale: scale,
                                      child: Icon(
                                        _iconForLabel(item.label, isActive: isActive),
...
IconData _iconForLabel(String label, {bool isActive = false}) {
  ...
  if (text.contains('account') || text.contains('profile') || text.contains('user') || text.contains('person') || text.contains('you') || text.contains('me')) return isActive ? Icons.person_rounded : Icons.person_outline_rounded;
  if (text.contains('category') || text.contains('categories') || text.contains('explore') || text.contains('browse') || text.contains('menu')) return isActive ? Icons.grid_view_rounded : Icons.grid_view_outlined;
...
String _normalizeUrl(String url) {
    if (url.isEmpty) return "ROOT";
    return url.toLowerCase().replaceAll(RegExp(r'/$'), '').trim();
}
