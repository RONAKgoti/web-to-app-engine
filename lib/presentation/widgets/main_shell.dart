...
class _MobileShellState extends State<MobileShell> {
  ...
  Widget build(BuildContext context) {
    ...
    // Fix for Myntra/Amazon Login: Hide bar when keyboard is active
    final bool isKeyboardVisible = MediaQuery.of(context).viewInsets.bottom > 0;

    return Scaffold(
      extendBody: true,
      resizeToAvoidBottomInset: false, // Don't push the whole app up
...
IconData _iconForLabel(String label) {
  ...
  if (text.contains('bag') || text.contains('shop') || text.contains('store')) return Icons.shopping_bag_outlined;
  ...
  if (text.contains('contact') || text.contains('support') || text.contains('customer') || text.contains('help')) return Icons.support_agent_outlined;
  ...
  // High-Tech & Software (Universal Professional)
  if (text.contains('web') || text.contains('site')) return Icons.language_rounded;
  if (text.contains('app') || text.contains('mobile') || text.contains('android') || text.contains('ios')) return Icons.smartphone_rounded;
  if (text.contains('cloud') || text.contains('salesforce')) return Icons.cloud_queue_rounded;
  if (text.contains('setting') || text.contains('config')) return Icons.settings_suggest_outlined;
  if (text.contains('dashboard') || text.contains('panel')) return Icons.dashboard_customize_outlined;
  if (text.contains('analytic') || text.contains('report') || text.contains('data')) return Icons.analytics_outlined;

  // E-commerce Specifics (Stability Focus - Amazon/Myntra Fix)
  if (text.contains('sign in') || text.contains('login') || text.contains('account') || text.contains('profile') || text.contains('your')) return Icons.person_outline_rounded;
  if (text.contains('deals') || text.contains('offer') || text.contains('coupons')) return Icons.local_offer_outlined;
  if (text.contains('orders') || text.contains('buying') || text.contains('purchase')) return Icons.local_mall_outlined;
  if (text.contains('category') || text.contains('categories') || text.contains('browse')) return Icons.subject_rounded;
  if (text.contains('pay') || text.contains('wallet') || text.contains('money')) return Icons.account_balance_wallet_outlined;
