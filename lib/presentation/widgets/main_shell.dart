...
class _MobileShellState extends State<MobileShell> {
  ...
  Widget build(BuildContext context) {
    ...
    // Slots 2-5: DYNAMICALLY populate from website categories
    ...
    // FILLER: If website has < 4 categories, fill with App pillars
    const appPillars = [
      _NavItem(icon: Icons.assistant_outlined, activeIcon: Icons.assistant_rounded, label: 'AI Tools', branchIndex: 1),
      _NavItem(icon: Icons.grid_view_outlined, activeIcon: Icons.grid_view_rounded, label: 'Hub', branchIndex: 2),
      _NavItem(icon: Icons.person_outline_rounded, activeIcon: Icons.person_rounded, label: 'Profile', branchIndex: 3),
    ];

    for (final pillar in appPillars) {
      if (items.length >= 5) break;
      items.add(pillar);
    }

    final finalItems = items.take(5).toList();
