...
class _HomeScreenState extends ConsumerState<HomeScreen> {
    ...
    controller.setNavigationDelegate(
        NavigationDelegate(
          ...
          onPageStarted: (url) {
            ...
            _setLoading(true);
            _updateCurrentUrl(url);
            // INJECT CONTINUOUS UI CLEANER (For Flipkart/Myntra/Professional Sites)
            _controller?.runJavaScript(r'''
               (function() {
                 const kill = () => {
                    ...
                    selectors.forEach(sel => {
                        document.querySelectorAll(sel).forEach(el => {
                            // Only kill if it's actually a bottom-fixed element
                 ...
                 // Run immediately and then every 800ms to catch dynamic re-renders
                 kill();
  ...
  _ScrapeResult _decodeScrapeResultFromJs(Object result) {
    final normalized = _normalizeJsResult(result);
    try {
      final decoded = json.decode(normalized);
      if (decoded is Map) {
         final itemsRaw = decoded['items'];
         final items = (itemsRaw is List) ? itemsRaw.whereType<Map>().map((i) => {
            'label': (i['label'] ?? '').toString(),
            'url': (i['url'] ?? '').toString(),
         }).where((i) => i['label']!.isNotEmpty && i['url']!.isNotEmpty).toList() : <Map<String, String>>[];
         
         return _ScrapeResult(
            items: items,
            logoUrl: decoded['logoUrl']?.toString(),
            themeColor: decoded['themeColor']?.toString(),
...
class _ScrapeResult {
  ...
  final String? title;
  _ScrapeResult({required this.items, this.logoUrl, this.themeColor, this.title});
}
