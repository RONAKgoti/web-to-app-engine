...
class _HomeScreenState extends ConsumerState<HomeScreen> {
    ...
    try {
      const script = r'''
        (() => {
          ...
          const hasMenuTrigger = (el) =>
            el.querySelector(
              'button, [role="button"], summary, [aria-label*="menu" i], [class*="menu-toggle"], [class*="menuicon"], [class*="hamburger"], [class*="drawer-toggle"], [class*="navbar-toggler"], [class*="mobile-toggle"], [class*="nav-toggle"]'
            ) !== null;

          const hasCompactBrandAsset = (el) =>
            Array.from(el.querySelectorAll('img, svg')).some((media) => {
          ...
          const hasLargeMedia = (el) => {
            for (const media of Array.from(el.querySelectorAll('img, picture, video, canvas, iframe'))) {
              const rect = media.getBoundingClientRect();
              if (rect.width >= window.innerWidth * 0.45 && rect.height >= 120) {
                return true;
              }
            }
            return false;
          ...
          const isStrongContentBlock = (el) => {
            if (!el) return false;
            const rect = el.getBoundingClientRect();
            const text = textFor(el);
            if (looksLikeContent(el, rect, text)) return true;
            if (hasLargeMedia(el) && rect.height >= 140) return true;
            if (el.querySelector('h1, h2, h3, article, section, [class*="hero"], [class*="banner"]') !== null &&
                (text.length > 60 || rect.height > 160)) {
              return true;
            }
            return false;
          };

          const isLikelySpacerBlock = (el) => {
            if (!el) return false;
            const rect = el.getBoundingClientRect();
            if (rect.width <= 0 || rect.height <= 0) return false;
            if (rect.top > window.innerHeight * 0.5) return false;
            if (rect.height > Math.max(280, window.innerHeight * 0.34)) return false;
            if (isStrongContentBlock(el)) return false;

            const computed = window.getComputedStyle(el);
            const text = textFor(el);
            const hasButtons = el.querySelectorAll('button, [role="button"], a[href]').length;
            const hasBackgroundImage = computed.backgroundImage && computed.backgroundImage !== 'none';

            return !hasBackgroundImage && text.length < 40 && hasButtons <= 1;
          };
          ...
          const isLikelyChromeBar = (el) => {
            ...
            const hasBrandAsset = hasLogoImage || hasCompactBrandAsset(el);
            const pinned = computed.position === 'fixed' || computed.position === 'sticky';
            ...
            const compactBrandBar = nearTop &&
              limitedHeight &&
              text.length <= 80 &&
              hasBrandAsset &&
              (hasMenuTrigger(el) || actionCount >= 1 || linkCount >= 1 || el.querySelectorAll('svg').length >= 1);
          ...
          const findMainContent = () => {
            ...
            for (const el of candidates) {
              ...
              const hasHeroMedia = hasLargeMedia(el);
              if (!nearTop) continue;
          ...
          const hidePrecedingChrome = () => {
            ...
            while (node && node !== document.body) {
              ...
              let hiddenCount = 0;
              while (sibling && hiddenCount < 8) {
                const previous = sibling.previousElementSibling;
                if (isLikelyChromeBar(sibling) || isLikelySpacerBlock(sibling)) {
                  hideElement(sibling);
                  hiddenCount += 1;
                }
                sibling = previous;
          ...
          const hideLeadingChildren = () => {
            ...
            const containers = [
              main.parentElement,
              main.parentElement ? main.parentElement.parentElement : null,
            ].filter(Boolean);

            for (const container of containers) {
              for (const child of Array.from(container.children).slice(0, 8)) {
                if (!child) continue;
                if (child === main || child.contains(main) || isStrongContentBlock(child)) {
                  break;
                }
                if (isLikelyChromeBar(child) || isLikelySpacerBlock(child)) {
                  hideElement(child);
                }
              }
            }
          ...
          const collapseTopSpacers = () => {
            ...
            while (sibling) {
              ...
              if ((rect.height < 180 && text.length === 0 && !hasMedia) ||
                  isLikelySpacerBlock(sibling)) {
                hideElement(sibling);
              }
              sibling = previous;
            }
          ...
          const normalizeMainSpacing = () => {
            ...
            while (node && node !== document.body && depth < 6) {
              ...
              node.style.setProperty('scroll-margin-top', '0px', 'important');
              node = node.parentElement;
              depth += 1;
            }
          };
          ...
          const alignMainContent = () => {
            const main = findMainContent();
            if (!main) return;
            normalizeMainSpacing();

            const rect = main.getBoundingClientRect();
            if (window.scrollY <= 24 && rect.top > 16) {
              window.scrollTo({
  ...
  void _applyMenuItems(List<Map<String, String>> rawItems,
    ...
    final shouldPreserveExisting = (items.length <= 1 && existing.length > 1) ||
        (items.length < 3 && existing.length >= 3);
  ...
  bool _isLikelyPrimaryNavigationItem(WebItem item, Uri siteUri) {
    ...
    if (label.isEmpty || uri == null || !_belongsToSameSite(siteUri, uri)) {
      return false;
    }
    ...
    if (uri.query.isNotEmpty && !hasGenericSectionKeyword) {
      return false;
    }
  ...
  int _menuPriorityScore(WebItem item, Uri siteUri) {
    final label = item.label.trim().toLowerCase();
    final uri = Uri.tryParse(item.url);
    final depth = uri == null ? 10 : _urlDepth(uri);
    var score = 100;
    ...
    if (uri != null && _belongsToSameSite(siteUri, uri)) {
      if (depth == 0) {
        score += 140;
      } else if (depth == 1) {
        score += 110;
  ...
  List<WebItem> _primaryNavigationItems(WebState state) {
    ...
    final siteUrl = currentUrl ?? initialUrl ?? state.currentUrl;
    final siteUri = Uri.tryParse(siteUrl ?? '');
    if (siteUri == null) {
