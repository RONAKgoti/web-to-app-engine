    ...
    try {
      const js = r'''
        (() => {
          ...
          const selectors = [
            'header', 'footer', 'nav', 'aside', '.wpadminbar', '#wpadminbar',
            '.elementor-location-header', '.elementor-location-footer',
            '.td-header-wrap', '.td-footer-wrap', '.tp-bullets', '.rev_slider_wrapper'
          ];
          ...
          document.querySelectorAll('body > *').forEach(el => {
            ...
            // Only hide bars at top/bottom that are NOT the main content
            const isTopBar = rect.top <= 80 && rect.height < 180;
            const isBottomBar = rect.bottom >= (h - 150) && rect.height < 250;
            const isSticky = (style.position === 'fixed' || style.position === 'sticky') && (rect.top <= 100 || rect.bottom >= (h - 100));
            ...
            if ((isTopBar || isBottomBar || isSticky) && !isMainArea && !hasSubstantialText) {
                // Heuristic filtering: Only hide if it looks like a menu/nav/footer
                const identity = (el.className + el.id).toLowerCase();
                if (/nav|menu|header|footer|tab|bar|social|widget|burger|logo/.test(identity)) {
                   el.style.setProperty('display', 'none', 'important');
          ...
          if (!document.getElementById(SCRAPER_ID)) {
            ...
            css.id = SCRAPER_ID;
            css.innerHTML = `
              .wpadminbar, #wpadminbar, .elementor-location-header, .elementor-location-footer,
              [class*="td-header-wrap"], [class*="td-footer-wrap"], .tp-bullets {
