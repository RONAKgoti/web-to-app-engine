      ...
      const script = r'''
        (() => {
          ...
          const isLikelyTopChrome = (el) => {
            ...
            return nearTop &&
              limitedHeight &&
              (keywordMatch || socialMatch || phoneMatch || compactBrandBar || linkCount >= 3 || (pinned && (linkCount >= 2 || actionCount >= 1)));
          };
          ...
          const findMainContent = () => {
            ...
            for (const el of candidates) {
              ...
              if (isLikelyTopChrome(el)) continue;
            ...
            for (const el of Array.from(document.body.children)) {
              ...
              const score =
                ...
                (rect.height * 2) +
                (el.querySelectorAll('img, p, h1, h2, h3, section, article').length * 20);
              if (score > bestScore && !isLikelyTopChrome(el)) {
                best = el;
                bestScore = score;
              }
            }
          ...
          const hideLeadingChildren = () => {
            ...
            for (const child of candidates) {
              ...
              if (hasStrongContent && !isLikelyTopChrome(child)) {
                contentFound = true;
                break;
              }

              if (!contentFound && isLikelyTopChrome(child)) {
                hideElement(child);
              }
            }
          };
