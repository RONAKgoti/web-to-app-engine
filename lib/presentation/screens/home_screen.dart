      ...
      const script = r'''
        (() => {
          ...
          const scoreContainer = (container, items) => {
            ...
            let appNavBoost = 0;
            const appWords = ['home', 'category', 'categories', 'deal', 'offer', 'account', 'profile', 'bag', 'cart', 'shop', 'order', 'sign', 'login', 'men', 'women', 'kid'];
            for(const item of items) {
              const lbl = item.label.toLowerCase();
              if(appWords.some(w => lbl.includes(w))) appNavBoost += 180;
              // Specific bonus for short, 'Action' labels common in apps
              if(lbl.length > 2 && lbl.length < 12) appNavBoost += 25;
            }

            const textPenalty = Math.max(0, text.length - 250);
            const sizePenalty = items.length > 8 ? (items.length - 8) * 35 : 0;
