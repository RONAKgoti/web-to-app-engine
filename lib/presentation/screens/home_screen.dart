...
class _HomeScreenState extends ConsumerState<HomeScreen> {
    ...
    try {
      const script = r'''
        (() => {
          ...
          const textFor = (el) => `${el.innerText || ''} ${el.id || ''} ${typeof el.className === 'string' ? el.className : ''}`
            .replace(/\s+/g, ' ')
            .trim()
            .toLowerCase();
          ...
          const looksLikeTopChrome = (el) => {
            ...
            const shortBand = rect.height <= 320;
            const pinned = computed.position === 'fixed' || computed.position === 'sticky';
            const keywordMatch = /header|topbar|top-bar|navbar|nav|menu|toolbar|masthead|social|contact|call|phone|logo/.test(text);
            const hasPhoneOrSocial = /facebook|instagram|twitter|linkedin|pinterest|youtube|\+\d|\bcall\b|\bcontact\b/.test(text);

            return nearTop && wide && shortBand && (keywordMatch || hasPhoneOrSocial || (pinned && (hasLinkCluster || hasButtons)));
          };
          ...
          const hideHeaderSiblingsBeforeMain = () => {
            const main = document.querySelector('main, [role="main"], #content, #main-content, .site-content, .entry-content, article, .elementor');
            if (!main) return;
            ...
            while (sibling) {
              ...
              const looksLikeChrome = rect.height <= Math.min(window.innerHeight * 0.7, 420) &&
                (sibling.querySelectorAll('a').length >= 2 || /header|nav|menu|social|contact|call|phone|logo/.test(text));
              if (looksLikeChrome) {
                hideElement(sibling);
              }
              sibling = previous;
          ...
          const removeEmptySpacers = () => {
            for (const el of Array.from(document.querySelectorAll('section, div'))) {
              ...
              const isSpacer = !hasMedia && !hasBackgroundImage && text.length === 0 && rect.height >= 70;
          ...
          const hideChrome = () => {
            document.querySelectorAll(hardSelectors.join(',')).forEach(hideElement);
          ...
          if (!window.__nativeAppShellObserver) {
            const observer = new MutationObserver(() => window.requestAnimationFrame(hideChrome));
  ...
  Widget _buildLoadingOverlay() {
    final value = loadingProgress > 0 && loadingProgress < 100
        ? loadingProgress / 100
        : null;
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Preparing a clean and responsive app view for your website.',
                textAlign: TextAlign.center,
                style: TextStyle(
                  fontSize: 13,
                  height: 1.5,
    ...
    if (_usesWebView) {
      ...
      return Stack(
        children: [
          ...
          if (showLoadingCover) Positioned.fill(child: _buildLoadingOverlay()),
