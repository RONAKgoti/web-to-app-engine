...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  String _expertCleanHtml(String html) {
    ...
    // 2. Decorative Cleanup (Be cautious)
    document.querySelectorAll('*').forEach((e) {
      String classes = e.className.toLowerCase();
      if (['pattern', 'dots', 'blob', 'spacer'].any((token) => classes.contains(token))) {
        if (e.text.trim().length < 10) e.remove();
      }
    });

    // 3. Smart Content Extraction
    // We try to find the biggest content block instead of just the first one
    var containers = document.querySelectorAll('main, article, .content, #content, .entry-content, .page-content, .et_pb_section, .elementor-section');
    dom.Element? main;
    
    if (containers.isNotEmpty) {
      // Pick the container with the most text to avoid picking small utility blocks
      main = containers.reduce((curr, next) => curr.text.length > next.text.length ? curr : next);
    } else {
      main = document.body;
    }
    ...
    // 4. Image Proportions & Spacing
    main.querySelectorAll('img').forEach((img) {
      String src = img.attributes['src']?.toLowerCase() ?? '';
      if (src.contains('arrow') || src.contains('icon') || src.contains('logo')) {
        img.attributes['style'] = 'width: 24px; height: 24px; object-fit: contain;';
      } else {
        img.attributes['style'] = 'max-width: 100%; height: auto; border-radius: 12px; margin: 12px 0;';
      }
    });
  ...
  void _parseNavigation(String html) {
    ...
    // Find the main menu container universally
    ...
    // 1. Mandatory Single Home
    allItems.add(WebItem(label: 'Home', url: homeUrl, icon: Icons.home_rounded));
    addedUrls.add(homeUrl.toLowerCase().replaceAll(RegExp(r'/$'), ''));

    if (nav != null) {
      // Find top-level LI items to check for children
      var topItems = nav.querySelectorAll(':scope > ul > li, :scope > div > ul > li, li.menu-item');
      
      for (var li in topItems) {
        var anchor = li.querySelector('a');
        if (anchor == null) continue;

        String label = anchor.text.trim();
        String? href = anchor.attributes['href'];
        if (label.isEmpty || label.length > 25 || href == null || href.startsWith('#')) continue;
        ...
        // CHECK FOR SUB-MENU (Dropdown)
        List<WebItem> subItems = [];
        var subMenu = li.querySelector('ul, .sub-menu, .dropdown-menu');
        if (subMenu != null) {
