...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  bool _isPagePresentationReady = false;
  late Dio _dio;
  ...
  WebViewController _buildWebViewController() {
    ...
    controller.addJavaScriptChannel(
      ...
      onMessageReceived: (JavaScriptMessage message) {
        ...
        } else if (msg == 'native_bar_found') {
           ref.read(webProvider.notifier).setHasNativeWebBar(true);
        }
      },
    );
    ...
    controller.setNavigationDelegate(
        NavigationDelegate(
          ...
          onPageStarted: (url) {
            ...
            _setLoading(true);
            _updateCurrentUrl(url);
            ref.read(webProvider.notifier).setHasNativeWebBar(false);
            // INJECT INSTANT DIRECT WATCHER (MutationObserver for Zero-Delay Detection)
            _controller?.runJavaScript(r'''
               (function() {
                 let detected = false;
                 const check = () => {
                    const selectors = ['nav', 'footer', '.bottom-nav', '[class*="bottom-nav"]', '[class*="bottomNav"]', '[class*="_2atY9r"]', '.fixed-bottom', '.mobile-tabs'];
                    for (const sel of selectors) {
                        const el = document.querySelector(sel);
                        if (el) {
                            const rect = el.getBoundingClientRect();
                            const style = window.getComputedStyle(el);
                            const isBottom = (style.position === 'fixed' || style.position === 'sticky') && (rect.bottom >= (window.innerHeight - 150));
                            if (isBottom) {
                                NativeAppChannel.postMessage('native_bar_found');
                                return true;
                            }
                        }
                    }
                 ...
                 if (check()) { detected = true; watcher.disconnect(); }
               })();
            ''').catchError((_){});

            // INJECT CONTINUOUS UI CLEANER (For Sites WITHOUT a Native Bar)
            final bool hasNativeShell = ref.read(webProvider).hasNativeWebBar;
            if (!hasNativeShell) {
              _controller?.runJavaScript(r'''
                 (function() {
                   const kill = () => {
                      const selectors = [
                          'nav', 'footer', '.bottom-nav', '[class*="bottom-nav"]', '[class*="bottomNav"]', 
                          '[class*="MobileBottom"]', '[id*="bottom-nav"]', '[data-testid*="nav"]',
                          '[class*="_2atY9r"]', '[class*="_2Xv9_t"]', '[class*="_1AtVbE"]', 
                          '.fixed-bottom', '#mobile-footer', '.mobile-tabs', '.header-container--fixed'
                      ];
            ...
            setState(() {
              loadingProgress = 0;
              _isPagePresentationReady = false;
            });
          },
  ...
  Future<void> _handleBackPressed() async {
    ...
    if (!_usesWebView && _history.isNotEmpty) {
      final previousUrl = _history.removeLast();
      await _fetchPage(previousUrl, addToHistory: false);
      return;
    }

    // Double-tap to exit logic (Professional App Standard)
  ...
  String _expertCleanHtml(String html, String baseUrl) {
    ...
    main!.querySelectorAll('img').forEach((img) {
      ...
      bool isLogo = src.contains('logo') || src.contains('brand') || src.contains('navbar');
      bool small = src.contains('icon') ||
          src.contains('tick') ||
          (img.attributes['width'] != null &&
              int.tryParse(img.attributes['width']!) != null &&
              int.parse(img.attributes['width']!) < 40);
              
      if (isLogo) {
        img.attributes['style'] =
            'height: 48px; width: auto; max-width: 180px; object-fit: contain; display: block; margin: 12px 0;';
      } else if (small) {
        img.attributes['style'] =
            'width: 28px; height: 28px; object-fit: contain; display: inline-block; vertical-align: middle; margin: 4px;';
      } else {
        img.attributes['style'] =
            'width: 100%; height: auto; border-radius: 20px; margin: 24px 0; display: block; box-shadow: 0 10px 30px rgba(0,0,0,0.05);';
      }
    });

    // 6. Text Normalization (Prevent HUGE fonts from breaking layout)
    main!.querySelectorAll('p, span, div, li, h1, h2, h3').forEach((el) {
       final style = el.attributes['style'] ?? '';
       if (style.contains('font-size')) {
          // Replace oversized font-sizes (>24px) with a maximum cap
          el.attributes['style'] = style.replaceAll(RegExp(r'font-size:[^;]+;?'), 'font-size: inherit;');
       }
    });
    ...
    try {
      const script = r'''
        (() => {
          ...
          const invalidLabelPattern = /security|sitemap|compliance|epr|read more|learn more|view more|click here|book now|download|watch now|apply now|buy now|see all|menu|back|next|previous|prev|close|help|support|contact|about|t&c|privacy|legal|terms|policy/i;
          ...
          const toItem = (element) => {
            ...
            if (!href || /^mailto:|^tel:/i.test(href) || href === '#' || href === 'javascript:void(0)' || href === 'javascript:;') return null;
          ...
          const allBestItems = [];
          // If we find a very high-quality container (like a real bottom nav), prioritize it alone
          const topOne = ranked[0];
          if (topOne && topOne.score > 1000) {
             topOne.container.setAttribute('data-native-app-chrome', '1');
             allBestItems.push(...topOne.items);
          } else {
            ranked.slice(0, 2).forEach((entry) => {
              entry.container.setAttribute('data-native-app-chrome', '1');
              allBestItems.push(...entry.items);
            });
          }
          ...
          const getThemeColor = () => {
            ...
            if (header) {
              ...
              if (bg !== 'rgba(0, 0, 0, 0)' && bg !== 'transparent' && bg !== 'rgb(255, 255, 255)') return bg;
            }
            return null;
          };

          const getLogo = () => {
            const img = document.querySelector('img[src*="logo"], [class*="logo"] img, [id*="logo"] img, .navbar-brand img, .site-logo img');
            return img ? img.src : null;
          };

          const labels = allBestItems.map(i => i.label.toLowerCase());
          const hasTwoCore = (labels.includes('home') || labels.includes('account') || labels.includes('profile')) && (labels.includes('cart') || labels.includes('bag') || labels.includes('categories') || labels.includes('search') || labels.includes('shop'));
          
          return JSON.stringify({
            items: allBestItems.slice(0, 24),
            themeColor: getThemeColor(),
            logoUrl: getLogo(),
            title: document.title,
            hasNativeWebBar: (topOne && topOne.score > 1200) || (hasTwoCore && ranked.some(r => r.score > 600)) || /flipkart|nykaa|myntra|amazon|ebay|aliexpress/.test(window.location.hostname)
          });
        })();
      ''';
      ...
      final scrapeRes = _decodeScrapeResultFromJs(result);
      
      if (scrapeRes.logoUrl != null) {
        ref.read(webProvider.notifier).setLogo(scrapeRes.logoUrl);
      }

      // Final Master Touch: Seamless Status Bar
      ...
      if (scrapeRes.title != null && scrapeRes.title!.isNotEmpty) {
        setState(() => pageTitle = _sanitizeTitle(scrapeRes.title));
      }

      ref.read(webProvider.notifier).setHasNativeWebBar(scrapeRes.hasNativeWebBar);
      _applyMenuItems(scrapeRes.items, activeUrl: activeUrl);
    } catch (e) {
      debugPrint('Navigation sync failed: $e');
    }
  }

  _ScrapeResult _decodeScrapeResultFromJs(dynamic result) {
    try {
      if (result == null) return _ScrapeResult(items: []);
      final String decoded = result is String ? result : result.toString();
      final Map<String, dynamic> data = json.decode(decoded);
      final List<dynamic> rawItems = data['items'] ?? [];
      final List<Map<String, String>> items = rawItems.map((item) {
        return {
      ...
      return _ScrapeResult(
        ...
        hasNativeWebBar: data['hasNativeWebBar'] == true
      );
    } catch (e) {
      debugPrint("JS Decode Error: $e");
      return _ScrapeResult(items: []);
    }
  }

  Future<void> _preparePageForDisplay() async {
    await _applyNativeChromeHiding();
    // Reduced delays to prevent flickering "jabakara"
      ...
      const js = r'''
        (() => {
          ...
          const selectors = [
            ...
            '.bnav', '.bottomBar', '.footer-nav', '.mobile-nav-bar',
            '.fixed-bottom', '.nav-bar-bottom', '[class*="tab-bar"]',
            /* E-commerce specific bottom bar removals */
            '.mobile-footer', '.mobile-navigation', '.am-mobile-menu'
          ];

          selectors.forEach(sel => {
            document.querySelectorAll(sel).forEach(el => {
               const style = window.getComputedStyle(el);
               if (style.position === 'fixed' || style.position === 'sticky' || el.getBoundingClientRect().top > (window.innerHeight * 0.7)) {
          ...
          document.querySelectorAll('body *').forEach(el => {
            ...
            const isBottomBar = rect.bottom >= (h - 110) && rect.height < 250;
            const isFixedBottom = (style.position === 'fixed' || style.position === 'sticky') && (rect.bottom >= (h - 110)) && rect.height < 250;
            const isTabStyle = /nav|menu|tab|footer|bar/.test((el.className + el.id).toLowerCase());

            const isProfilePage = /login|account|profile|signup|otp|verify/i.test(window.location.href);
            
            // CRITICAL: If on a Profile/Login page, SHUT DOWN the hider immediately
            // This prevents the "White Screen" or "Hanging" sensation you reported.
            if (isProfilePage) return;

            if ((isBottomBar || isFixedBottom || isTabStyle) && !isMainArea && !hasSubstantialText && rect.top > (h * 0.45)) {
                el.style.setProperty('display', 'none', 'important');
          ...
          if (!document.getElementById(SCRAPER_ID)) {
             ...
             css.innerHTML = `
                ...
                /* SMART TEXT & NATIVE FEEDBACK */
                h1, .entry-title, .post-title { font-size: 26px !important; line-height: 1.25 !important; }
                h2 { font-size: 22px !important; line-height: 1.3 !important; }
                * { -webkit-tap-highlight-color: transparent !important; }
                a, button, nav, .menu, footer { user-select: none !important; }
                article, section, p, h1, h2, h3 { user-select: text !important; }
                ...
                /* MINIMAL LOGO FIX */
                [class*="logo"] img, [id*="logo"] img, img[src*="logo"], .navbar-brand img, .site-logo img {
             ...
             // 5. Intelligent Dynamic Text Scaler (Fix for Monster headers)
             const normalizeText = () => {
                document.querySelectorAll('h1, h2, h3, h4, h5, h6, div, span, p, a').forEach(el => {
                   const style = window.getComputedStyle(el);
                   const fs = parseFloat(style.fontSize);
                   if (fs > 26) {
                      el.style.setProperty('font-size', 'clamp(20px, 7vw, 26px)', 'important');
             ...
             normalizeText();
             setTimeout(normalizeText, 1500); // Handle lazy-loaded content
  ...
  void _applyMenuItems(List<Map<String, String>> rawItems,
    ...
    final filteredRaw = rawItems.where((item) {
      ...
      const junkTerms = ['security', 'sitemap', 'compliance', 'epr', 'support', 'help', 'contact', 'legal', 'privacy', 'policy', 'return', 'refund', 'terms', 'conditions', 'career', 'shipping', 'about', 'blog', 'faq'];
      return !junkTerms.any((word) => lbl.contains(word));
    }).toList();
    ...
    int _getMenuQuality(List<WebItem> menu) {
      ...
      final premiumWords = ['category', 'deal', 'offer', 'cart', 'bag', 'buy', 'shop', 'pay', 'order', 'wishlist', 'profile', 'account', 'login', 'user', 'sign in', 'me'];
      final junkWords = ['your', 'customer', 'support', 'help', 'career', 'shipping', 'privacy', 'policy', 'return', 'refund', 'cookie', 'legal', 'term', 'condition', 'track'];
  ...
  Widget _buildBrowserSurface(WebState state) {
    if (_usesWebView) {
      ...
      final showLoadingSkeleton =
          errorMessage == null && (state.isLoading || !_isPagePresentationReady) && loadingProgress < 75;
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, result) {
        if (didPop) return;
        _isPagePresentationReady = false; // Reset for next page
        _handleBack();
      },
      child: Scaffold(
        key: _scaffoldKey,
