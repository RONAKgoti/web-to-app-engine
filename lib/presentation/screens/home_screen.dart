    ...
    try {
      const js = r'''
        (() => {
          ...
          // 1. Target by common selectors (Surgical Stripping)
          const selectors = [
            'header', 'footer', 'nav', 'aside', '.nav', '.header', '.footer',
            '[class*="header"]', '[class*="footer"]', '[class*="nav"]', '[class*="menu"]',
            '[id*="header"]', '[id*="footer"]', '[id*="nav"]', '[id*="menu"]',
            '.wpadminbar', '#wpadminbar', '.sticky-header', '.sticky-footer',
            '.mobile-header', '.mobile-footer', '.mobile-menu', '.mobile-nav',
            '.elementor-location-header', '.elementor-location-footer',
            '.tp-bullets', '.rev_slider_wrapper', '.whatsapp-button', '.chat-widget'
          ];

          selectors.forEach(sel => {
            document.querySelectorAll(sel).forEach(el => {
              // Protecting Main Content: Never hide if it's likely the body content
              if (el.matches('main, article, #main, .main, #content, .content, #main-content')) return;
              if (el.innerText.length > 800) return; // Likely a main container
          ...
          // 2. Target by Position Heuristics (Universal Auto-Detection)
          document.querySelectorAll('body > *').forEach(el => {
            const rect = el.getBoundingClientRect();
            const style = window.getComputedStyle(el);
            
            // A. Top/Bottom Bars (Universal Headers/Footers)
            const isTopBar = rect.top <= 100 && rect.height < (h * 0.25) && rect.width >= (w * 0.85);
            const isBottomBar = rect.bottom >= (h - 100) && rect.height < (h * 0.25) && rect.width >= (w * 0.85);
            const isSticky = style.position === 'fixed' || style.position === 'sticky';

            if ((isTopBar || isBottomBar || isSticky) && !el.matches('main, article, #content, .content, #main, .main')) {
               // Only hide if it lacks substantial content
          ...
          // 3. Persistent Global Style (Universal Injection)
          if (!document.getElementById(SCRAPER_ID)) {
            const css = document.createElement('style');
            css.id = SCRAPER_ID;
            css.innerHTML = `
              header, footer, nav, aside, .header, .footer, .nav, .menu,
              .mobile-menu, .mobile-nav, .wpadminbar, #wpadminbar,
              [class*="header"]:not(main):not(article), 
              [class*="footer"]:not(main):not(article),
              [id*="header"]:not(main):not(article), 
              [id*="footer"]:not(main):not(article),
              [class*="td-footer-wrap"], .elementor-location-header, .elementor-location-footer {
