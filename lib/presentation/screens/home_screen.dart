...
class _HomeScreenState extends ConsumerState<HomeScreen> {
      ...
      const script = r'''
        (() => {
          ...
          const isValidLabel = (label) => {
            ...
            return !/read more|learn more|view more|click here|book now|download|watch now/i.test(label);
          };
          ...
          const pushAnchor = (anchor) => {
            ...
            if (!isValidLabel(label) || !href) return;
            if (!/^https?:/i.test(href)) return;
            if (/facebook|instagram|twitter|linkedin|youtube|whatsapp|mailto:|tel:/i.test(href)) return;
          ...
          document.querySelectorAll(selectors.join(',')).forEach(pushAnchor);
      ...
      const script = r'''
        (() => {
          ...
          const isLikelyTopChrome = (el) => {
            ...
            const keywordMatch = /header|topbar|top-bar|nav|menu|toolbar|masthead|branding|social|contact|phone|call|logo/.test(identity);
            ...
            return nearTop &&
              limitedHeight &&
              (keywordMatch || socialMatch || phoneMatch || linkCount >= 3 || (pinned && (linkCount >= 2 || actionCount >= 1)));
          };
          ...
          const findMainContent = () => {
            ...
            for (const selector of selectors) {
              ...
              const rect = el.getBoundingClientRect();
              const text = textFor(el);
              if (rect.height > 140 || text.length > 120) {
                return el;
              }
            }
          ...
          const hidePrecedingChrome = () => {
            ...
            let hiddenCount = 0;
            while (sibling && hiddenCount < 5) {
              const previous = sibling.previousElementSibling;
              if (isLikelyTopChrome(sibling)) {
                hideElement(sibling);
                hiddenCount += 1;
              }
              sibling = previous;
  ...
  List<Map<String, String>> _extractNavigationItemsFromHtml(
    ...
    if (anchors.length < 3) {
      anchors.addAll(document.querySelectorAll('a[href]'));
    }
    ...
    for (final anchor in anchors) {
      ...
      if (RegExp(r'read more|learn more|view more|click here|book now|download',
              caseSensitive: false)
          .hasMatch(label)) {
        continue;
      }
  ...
  Widget _buildBottomNavigation(WebState state) {
    final hasUrl = (currentUrl ?? initialUrl) != null;
    final canOpenSections = state.menuItems.length > 1;

    final items = <_BottomBarItem>[
      _BottomBarItem(
        icon: Icons.home_rounded,
        label: 'Home',
        selected: _isOnHomePage,
        enabled: initialUrl != null,
      ...
      _BottomBarItem(
        ...
        selected: false,
        enabled: hasUrl,
        onTap: () => unawaited(_openInBrowser()),
      ),
      _BottomBarItem(
