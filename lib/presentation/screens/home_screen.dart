...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  String _expertCleanHtml(String html) {
    ...
    // 1. Technical Cleanup (Safe)
    document.querySelectorAll('script, style, link, meta, iframe, noscript, .wpadminbar').forEach((e) => e.remove());

    // 2. Target only SPECIFIC decorative elements that we know break UI
    // We avoid generic terms like 'bg' or 'background' now to prevent over-cleaning
    const decorativeTokens = ['pattern', 'dots', 'shape', 'decoration', 'divider', 'spacer', 'blob'];
    document.querySelectorAll('*').forEach((e) {
      String classes = e.className.toLowerCase();
      if (decorativeTokens.any((token) => classes.contains(token))) {
        // If it looks decorative AND has almost no text content, it's safe to remove
        if (e.text.trim().length < 20 && e.querySelectorAll('img').isEmpty) {
          e.remove();
        }
      }
    });
    ...
    // We look for common main containers, but fallback to <body> if not found
    ...
    // 4. Force Image Proportions (Universal Fix)
    main.querySelectorAll('img').forEach((img) {
      String src = img.attributes['src']?.toLowerCase() ?? '';
      bool isMiniIcon = src.contains('arrow') || src.contains('chevron') || src.contains('icon') || src.contains('logo');
      
      if (isMiniIcon) {
        img.attributes['style'] = 'width: 24px; height: 24px; object-fit: contain; vertical-align: middle;';
      } else {
        img.attributes['style'] = 'max-width: 100%; height: auto; border-radius: 12px; display: block; margin: 16px auto;';
      }
    });

    // 5. Remove empty tags that create massive white gaps
    main.querySelectorAll('div:empty, p:empty, span:empty, section:empty').forEach((e) => e.remove());
  ...
  void _parseNavigation(String html) {
    ...
    // Universal Menu Selector: Find any link list that looks like a main menu
    var menuIndices = [
      'nav ul li a', 
      '.menu-item a', 
      '.nav-link', 
    ...
    for (var selector in menuIndices) {
      var links = document.querySelectorAll(selector);
      for (var link in links) {
        String label = link.text.trim();
        String? href = link.attributes['href'];
        
        if (label.isEmpty || label.length < 2 || label.length > 20 || href == null || href.startsWith('#')) continue;
      ...
      if (allItems.length > 1) break; // Found a good menu
    }
