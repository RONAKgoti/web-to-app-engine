...
class _HomeScreenState extends ConsumerState<HomeScreen> {
      ...
      const js = r'''
        (() => {
          ...
          document.querySelectorAll('body *').forEach(el => {
            ...
            const isFixedBottom = (style.position === 'fixed' || style.position === 'sticky') && (rect.bottom >= (h - 150));
          ...
          // 3. Persistent Global Style (Safe Overrides)
          if (!document.getElementById(SCRAPER_ID)) {
            ...
            // Block generic context menu so images can't be deep-pressed like in a browser
  ...
  IconData _iconForLabel(String label) {
    ...
    if (text.contains('cart')) return Icons.shopping_cart_rounded;
    if (text.contains('product') || text.contains('collection')) return Icons.inventory_2_rounded;
    if (text.contains('offer') || text.contains('deal') || text.contains('sale')) return Icons.local_offer_rounded;
    ...
    if (text.contains('kid') || text.contains('infant')) return Icons.child_care_rounded;
    if (text.contains('beauty') || text.contains('fashion')) return Icons.face_retouching_natural_rounded;
    if (text.contains('accessories')) return Icons.watch_rounded;
    if (text.contains('brand')) return Icons.branding_watermark_rounded;
  ...
  Widget _buildAppDrawer(WebState state) {
    ...
    final filteredWebsiteMenu = <WebItem>[];
    final seenDrawerLabels = <String>{};
    
    for (final item in websiteMenuItems) {
       final key = _normalizeUrlKey(item.url);
       final isHome = key == _normalizeUrlKey(initialUrl ?? '');
       final label = item.label.toLowerCase();
       
       if (!isHome && !bottomBarUrls.contains(key) && !bottomBarLabels.contains(label)) {
          if (seenDrawerLabels.add(label)) {
             filteredWebsiteMenu.add(item);
             if (filteredWebsiteMenu.length >= 8) break;
          }
       }
    }
                ...
                child: Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text(
                        'We will auto-connect when network restores.',
            ...
            children: [
              ...
              Positioned(
                ...
                child: FloatingActionButton.small(
                  ...
                  child: const Icon(Icons.menu_rounded, color: Colors.transparent),
                ),
              ),
            ],
          ),
