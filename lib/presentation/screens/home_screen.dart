...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  Future<void> _fetchPage(String url) async {
    ...
    try {
      ...
      // Clear old content immediately to show loading state properly
      setState(() {
        htmlContent = null; 
      });
      ...
      if (response.statusCode == 200) {
        ...
        // Only parse nav from Home or if menu is empty
        if (ref.read(webProvider).menuItems.length <= 1 || url.length < (currentUrl?.length ?? 0)) {
  ...
  String _expertCleanHtml(String html) {
    ...
    // 1. Technical Noise Removal
    document.querySelectorAll('script, style, link, meta, iframe, noscript, .wpadminbar, #wpadminbar').forEach((e) => e.remove());

    // 2. Surgical Removal of Global UI (The "Sticky" stuff that clutters apps)
    // Targeting headers, footers, mobile menus, and social bars
    var globalUI = [
      'header', 'footer', 'nav', '.mobile-menu', '#mobile-menu', '.et_mobile_menu', 
      '.et_pb_menu', '.elementor-location-header', '.elementor-location-footer',
      '.top-bar', '.bottom-bar', '.social-links', '.social-icons', '.share-buttons'
    ];
    for (var selector in globalUI) {
      document.querySelectorAll(selector).forEach((e) => e.remove());
    }

    // 3. Decorative Element Cull (Dots, Shapes, Blobs)
    // These look like big photos in the app but are just background noise
    document.querySelectorAll('*').forEach((e) {
      String classes = e.className.toLowerCase();
      String id = e.id.toLowerCase();
      bool isDecorative = classes.contains('pattern') || classes.contains('shape') || 
                          classes.contains('divider') || classes.contains('spacer') ||
                          classes.contains('decoration') || classes.contains('blob') ||
                          id.contains('dots') || id.contains('circle');
      
      // If decorative AND has no real text, kill it
      if (isDecorative && e.text.trim().length < 30) {
        e.remove();
      }
    });

    // 4. Capture the Heart of the Page
    ...
    if (body == null) return "Engine Error: Content not found.";

    // 5. Image & Icon Discipline (Strict Proportions)
    body.querySelectorAll('img, svg').forEach((img) {
      String src = (img.attributes['src'] ?? '').toLowerCase();
      String classes = (img.className).toLowerCase();
      
      // Determine if it's a small UI element or a content photo
      bool isSmallIcon = src.contains('icon') || src.contains('logo') || src.contains('arrow') || 
                         src.contains('chevron') || classes.contains('icon') || 
                         (img.attributes['width'] != null && int.tryParse(img.attributes['width']!) != null && int.parse(img.attributes['width']!) < 50);

      if (isSmallIcon) {
        // UI Icons: Force tiny size, no full-width
        img.attributes['style'] = 'width: 32px !important; height: 32px !important; object-fit: contain; display: inline-block; vertical-align: middle; margin: 4px;';
        img.attributes['class'] = 'native-mini-icon';
      } else {
        // Content Photos: Professional Rounded Cards
        img.attributes['style'] = 'width: 100% !important; height: auto !important; border-radius: 20px; margin: 16px 0; display: block; box-shadow: 0 8px 30px rgba(0,0,0,0.08);';
      }
    });

    // 6. Remove excess whitespace created by empty wrappers
    body.querySelectorAll('div:empty, p:empty, section:empty').forEach((e) => e.remove());
  ...
  void _parseNavigation(String html) {
    ...
    // Core Home Entry (Always first)
    allItems.add(WebItem(label: 'Home', url: rootUrl, icon: Icons.home_rounded));
    addedUrls.add(rootUrl.toLowerCase().replaceAll(RegExp(r'/$'), ''));

    // Discover Menu Links
    var links = document.querySelectorAll('header a, nav a, .menu a, .nav-link, .elementor-nav-menu a');
    
    for (var link in links) {
      String label = link.text.trim();
      String? href = link.attributes['href'];
      
      if (label.isEmpty || label.length > 20 || label.length < 2 || href == null || href.startsWith('#')) continue;
