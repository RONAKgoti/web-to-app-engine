...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  String _expertCleanHtml(String html) {
    ...
    // 1. Surgical Technical Cleanup
    document.querySelectorAll('script, style, link, meta, iframe, noscript, .wpadminbar, #wpadminbar, .et_pb_menu').forEach((e) => e.remove());

    // 2. High-Precision Header/Footer Removal
    // We target ONLY the outermost global UI elements to preserve inner section headers
    var globalSelectors = [
      '#main-header', '#main-footer', '.et-main-header', '.et-main-footer',
      'header.site-header', 'footer.site-footer', '.top-bar', '.footer-bottom',
      '#top-header', '.et-top-navigation'
    ];
    for (var selector in globalSelectors) {
      document.querySelectorAll(selector).forEach((e) => e.remove());
    }

    // 3. Absolute Body Extraction (Hero-First)
    // We look for the absolute root of the content area
    ...
    if (content == null) return "Engine Error: Content shell not found.";

    // 4. Transform Images & Icons for Premium App Look
    content.querySelectorAll('img, svg').forEach((img) {
      ...
      bool isIcon = src.contains('icon') || src.contains('logo') || src.contains('chevron') || 
                    src.contains('arrow') || classes.contains('icon') || alt.contains('icon');
      
      if (isIcon) {
        // Utility icons: Scaled down
        img.attributes['style'] = 'width: 24px; height: 24px; object-fit: contain; vertical-align: middle; margin: 4px;';
        img.attributes['class'] = 'native-decorative-image';
      } else {
        // Main Content/Hero Images: Full width, Premium rounded corners
        img.attributes['style'] = 'width: 100%; height: auto; border-radius: 16px; margin: 16px 0; display: block; box-shadow: 0 4px 20px rgba(0,0,0,0.06);';
      }
    });

    // 5. Cleanup empty wrappers that cause layout gaps
    content.querySelectorAll('p:empty, div:empty').forEach((e) {
      if (e.text.trim().isEmpty && e.children.isEmpty) e.remove();
    });
  ...
  void _parseNavigation(String html) {
    ...
    // Universal Menu Discovery (WP, Divi, Elementor, Custom)
    var navSelectors = [
      'nav ul li a', 
      'header ul li a',
      '.menu-item a', 
    ...
    for (var selector in navSelectors) {
      ...
      if (allItems.length > 5) break; // Found a solid menu
    }
