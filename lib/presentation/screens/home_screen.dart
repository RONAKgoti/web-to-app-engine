      ...
      const js = r'''
        (() => {
          ...
          document.querySelectorAll('body *').forEach(el => {
            ...
            const isMainArea = el.matches('main, article, section, #main, .main, #content, .content, #main-content, [role="main"], .account-container, .profile-view');
            const hasSubstantialText = textLength > 100;
            const isProfilePage = window.location.href.toLowerCase().includes('account') || window.location.href.toLowerCase().includes('profile');
            
            if ((isBottomBar || isFixedBottom || isTabStyle) && !isMainArea && !hasSubstantialText && rect.top > (h * 0.65)) {
                // Never hide things on profile pages that might be essential
                if (!isProfilePage) {
                    el.style.setProperty('display', 'none', 'important');
