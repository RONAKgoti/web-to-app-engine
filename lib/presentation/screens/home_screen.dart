    ...
    controller.setNavigationDelegate(
        NavigationDelegate(
          ...
          onPageStarted: (url) {
            ...
            _setLoading(true);
            _updateCurrentUrl(url);
            // INJECT INSTANT Hider CSS (Aggressive mode for Flipkart/Myntra/E-commerce)
            _controller?.runJavaScript(r'''
               (function() {
                 ...
                 style.innerHTML = `
                    ...
                    [class*="_2Xv9_t"], [class*="_1AtVbE"], [class*="fixed-bottom"] { 
      ...
      const js = r'''
        (() => {
          ...
          document.querySelectorAll('body *').forEach(el => {
            ...
            // CRITICAL: Protection for Main Content & Profile Pages
            const isMainArea = el.matches('main, article, section, #main, .main, #content, .content, #main-content, [role="main"], .account-container, .profile-view, [class*="login"], [id*="login"], [class*="account"]');
            const hasSubstantialText = textLength > 80;
            const isProfilePage = /login|account|profile|signup|otp|verify/i.test(window.location.href);
            
            if ((isBottomBar || isFixedBottom || isTabStyle) && !isMainArea && !hasSubstantialText && rect.top > (h * 0.55)) {
                // Never hide things on profile/login pages that might be essential
                if (!isProfilePage) {
                    el.style.setProperty('display', 'none', 'important');
