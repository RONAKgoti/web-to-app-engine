...
class _HomeScreenState extends ConsumerState<HomeScreen> {
      ...
      const script = r'''
        (() => {
          ...
          const scoreContainer = (container, items) => {
            ...
            // Highly reward headers on E-commerce since they hold the cart!
  ...
  void _applyMenuItems(List<Map<String, String>> rawItems,
    ...
    int _getMenuQuality(List<WebItem> menu) {
      ...
      final premiumWords = ['category', 'deals', 'offer', 'cart', 'bag', 'buy', 'shop', 'pay', 'order'];
      final junkWords = ['your', 'customer', 'account', 'sign in', 'login', 'support', 'help', 'career'];
      
      int score = menu.length * 10;
      for (final item in menu) {
        final lbl = item.label.toLowerCase();
        if (premiumWords.any((w) => lbl.contains(w))) score += 50;
        if (junkWords.any((w) => lbl.contains(w))) score -= 30; // Weight down 'Account' bars
      }
      return score;
    }
    ...
    // Only update if:
    // 1. Current menu is extremely weak (empty or junk)
    // 2. We are on the home page AND the new menu is better or equal
    // 3. The new menu is objectively MORE 'Premium' than the old one
    final bool shouldOverwrite = !hasDecentExisting || 
                                (isHome && newScore >= existingScore) || 
                                (newScore > existingScore + 40);
