  ...
  void _smartAnalyze(String currentUrl) async {
    ...
    try {
      ...
      final List<SmartModule> master = [
        ...
        SmartModule(id: 'service', label: 'Services', icon: Icons.work_rounded),
        SmartModule(id: 'account', label: 'User', icon: Icons.person_rounded),
        SmartModule(id: 'contact', label: 'Help', icon: Icons.contact_support_rounded),
      ];

      for (var mod in master) {
        if (detected.containsKey(mod.id)) {
          found.add(SmartModule(id: mod.id, label: mod.label, icon: mod.icon, url: detected[mod.id]));
        }
      }
      ...
      if (found.length < 3) {
        found = [
          ...
          SmartModule(id: 'service', label: 'Works', icon: Icons.work_rounded, url: "$mainUrl/services"),
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        bottomNavigationBar: (state.activeTabs.length > 1) ? Container(
